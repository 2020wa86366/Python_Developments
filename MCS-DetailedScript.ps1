...
# ----------
$adminAddress = 'kf-cs-v7.kanuflemwiprolab.com' #The XD Controller we're going to execute against
$xdControllers = 'KF-CS-V7.kanuflemwiprolab.com'
...
$storageResource = "KF-Res" #Storage
$hostResource = "KF-Connect" #Hypervisor management

# Machine catalog properties
$machineCatalogName = "Windopf"
$machineCatalogDesc = "Windows 8.1 x86 SP1 with Office 2013"
$domain = "kanuflemwiprolab.com"
$orgUnit = "OU=WIN10Desktop,DC=kanuflemwiprolab,DC=com"
$namingScheme = "Wkop##" #AD machine account naming conventions
$namingSchemeType = "Numeric" #Also: Alphabetic
$allocType = "Random" #Also: Static
$persistChanges = "OnLocal" #Also: OnLocal, OnPvD
...
$identPool = New-AcctIdentityPool -AdminAddress $adminAddress -Domain $domain -IdentityPoolName $machineCatalogName -NamingScheme $namingScheme -NamingSchemeType $namingSchemeType 
...
$catalogUid = Get-BrokerCatalog -AdminAddress $adminAddress | Where-Object { $_.Name -eq $machineCatalogName } | Select-Object Uid
$guid = [guid]::NewGuid()
...
If (Test-ProvSchemeNameAvailable -AdminAddress $adminAddress -ProvisioningSchemeName @($machineCatalogName))
{
  ...
  { 
      ...
      $provScheme = Get-ProvScheme -AdminAddress $adminAddress | Where-Object { $_.ProvisioningSchemeName -eq $machineCatalogName }
      ...
      $adAccounts = New-AcctADAccount -AdminAddress $adminAddress -Count 5 -Ide $idenntityPoolUidtPool.IdentityPoolUid
